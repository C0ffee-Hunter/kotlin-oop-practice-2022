...
fun main() {
    ...
    val figureList_2 = ShapeCollector<ColoredShape2d>()
    figureList_2.addAll(figurelist.listAllFigures())
    figureList_2.getSorted(compareBy{it.calcArea()})
...
class ShapeCollector<T : ColoredShape2d> {
    ...
    fun searchType(key: String): ArrayList<T> {
        val typeList: ArrayList<T> = arrayListOf()
        val size = figureList.size - 1
        for (i in 0..size) {
